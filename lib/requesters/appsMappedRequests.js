...
function checkFinished (opts, savedApps, nextToken) {
  const maxLength = opts.start + opts.num;
  if (savedApps.length >= maxLength || !nextToken) {
    const result = savedApps.slice(opts.start, maxLength);
    return processFullDetailApps(result, opts);
  }
